...
import { getComputeTools, handleComputeTools } from './tools/compute-tools-readonly.js';
...
class OCIMCPServer {
  ...
  constructor() {
    this.isReadOnlyMode = OCIConfigService.isReadOnlyMode();
  ...
  private setupHandlers(): void {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      const computeTools = getComputeTools();
  ...
  async run(): Promise<void> {
    ...
    const computeTools = getComputeTools();
    const mode = this.isReadOnlyMode ? 'READ-ONLY' : 'FULL';
    
    // Log server info to stderr so it doesn't interfere with MCP protocol
    console.error(`OCI MCP Server running on stdio (${mode} mode)`);
    if (this.isReadOnlyMode) {
      console.error('🔒 Read-only mode enabled - destructive operations disabled');
      console.error('💡 Set OCI_MCP_READ_ONLY=false to enable full mode');
    }
